...
// const getAllProduct = (filter, page = 0, limit = null) => {
//     return new Promise(async (resolve, reject) => {
//         try {
//             const totalProduct = await Product.countDocuments();
...
//             if (filter) {
//                 const label = filter[0]
//                 const allProduct = await Product.find({ [label]: {'$regex': filter[1]} }).limit(limit).skip(page * limit).sort({createdAt: -1, updatedAt: -1})
